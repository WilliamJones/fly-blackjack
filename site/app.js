...
let clientId = store.get("fly-table-id") || ("c" + Math.random().toString(36).slice(2, 10));
...
let connLabel = "this tab only";
if (dbApi) { backend = new SharedStore(dbApi); connLabel = "shared"; }
else if (window.FIREBASE_CONFIG) {
  try {
    const { connectFirestore } = await import("./firebase-boot.js");
    backend = await connectFirestore(window.FIREBASE_CONFIG, myName);
    clientId = backend.uid;                    // identity is the signed-in user
    connLabel = "live";
  } catch (e) { console.warn("Firestore unavailable, falling back", e); }
}
if (!backend) backend = new LocalStore();
$("#conn").textContent = connLabel;
$("#conn").classList.toggle("live", connLabel !== "this tab only");

let brainLoaded = false;
backend.watchBrain(async b => {
  if (seated) return;                      // we are the writer; don't overwrite ourselves
  if (!b && !brainLoaded) {
    // nothing stored yet: begin from the fly that played 30,000 hands alone.
    // the first hand dealt at this table writes it to the store.
    try { b = await fetch("brain_seed.json").then(r => r.json()); } catch {}
  }
